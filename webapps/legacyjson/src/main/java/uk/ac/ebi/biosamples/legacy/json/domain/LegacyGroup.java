...
@JsonPropertyOrder(
    value = {"accession", "name", "releaseDate", "updateDate", "description", "characteristics"})
public class LegacyGroup {

  @JsonIgnore private final Sample sample;
  ...
  private Optional<String> extractSampleDescription(Sample sample) {
    Optional<Attribute> descriptionAttribute =
        this.sample.getAttributes().stream()
  ...
  private MultiValueMap<String, LegacyAttribute> extractCharacteristics(Sample sample) {
    Map<String, List<Attribute>> attributesByType =
    ...
    for (String type : attributesByType.keySet()) {
      legacyAttributesByType.put(
          LegacyJsonUtilities.camelCaser(type),
          attributesByType.get(type).stream()
  ...
  private static class LocalDateSerializer extends StdSerializer<Instant> {
    ...
    public void serialize(Instant value, JsonGenerator gen, SerializerProvider provider)
        throws IOException {
      gen.writeString(
          value.atZone(ZoneId.systemDefault()).format(DateTimeFormatter.ISO_LOCAL_DATE));
    }
  }
}
