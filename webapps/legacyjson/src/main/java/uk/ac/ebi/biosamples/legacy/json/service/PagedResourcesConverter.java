...
public class PagedResourcesConverter {
  ...
  public PagedResourcesConverter(
      PagedResourcesAssembler<GroupsRelations> groupsRelationsPagedResourcesAssembler,
      PagedResourcesAssembler<LegacySample> legacySamplePagedResourcesAssembler,
  ...
  public PagedResources<Resource<LegacySample>> toLegacySamplesPagedResource(
    ...
    List<LegacySample> legacyRelationsResources =
        samplePagedResources.getContent().stream()
            .map(Resource::getContent)
            .map(LegacySample::new)
            .collect(Collectors.toList());
    Page<LegacySample> pageRequest =
        buildPageRequest(legacyRelationsResources, samplePagedResources.getMetadata());
    return legacySamplePagedResourcesAssembler.toResource(
        pageRequest, legacySampleResourceAssembler);
  }

  public PagedResources<Resource<LegacyGroup>> toLegacyGroupsPagedResource(
      PagedResources<Resource<Sample>> samplePagedResources) {

    if (samplePagedResources == null || samplePagedResources.getContent().isEmpty()) {
      return getGroupsEmptyPagedResource();
    }
    List<LegacyGroup> legacyRelationsResources =
        samplePagedResources.getContent().stream()
            .map(Resource::getContent)
            .map(LegacyGroup::new)
            .collect(Collectors.toList());
    Page<LegacyGroup> pageRequest =
        buildPageRequest(legacyRelationsResources, samplePagedResources.getMetadata());
    return legacyGroupPagedResourcesAssembler.toResource(pageRequest, legacyGroupResourceAssembler);
  }

  public PagedResources<Resource<GroupsRelations>> toGroupsRelationsPagedResource(
      PagedResources<Resource<Sample>> samplePagedResources) {
    if (samplePagedResources == null || samplePagedResources.getContent().isEmpty()) {
      return getGroupsEmptyPagedResource();
    }

    List<GroupsRelations> legacyRelationsResources =
        samplePagedResources.getContent().stream()
            .map(Resource::getContent)
            .map(GroupsRelations::new)
            .collect(Collectors.toList());

    Page<GroupsRelations> pageRequest =
        buildPageRequest(legacyRelationsResources, samplePagedResources.getMetadata());
    return groupsRelationsPagedResourcesAssembler.toResource(
        pageRequest, groupRelationsResourceAssembler);
  }

  public PagedResources<Resource<ExternalLinksRelation>> toExternalLinksRelationPagedResource(
      PagedResources<Resource<Sample>> samplePagedResources) {
    ...
    //                .map(LegacyGroup::new)
    //                .collect(Collectors.toList());
    //        Page<LegacyGroup> pageRequest = buildPageRequest(legacyRelationsResources,
    // samplePagedResources.getMetadata());
    //        return legacyGroupPagedResourcesAssembler.toResource(pageRequest,
    // legacyGroupResourceAssembler);

  }

  private PagedResources getEmptyPagedResource(Class className) {
    return legacySamplePagedResourcesAssembler.toEmptyResource(
        new PageImpl<>(new ArrayList<>(), new PageRequest(0, 50), 0), className, null);
  }

  private PagedResources getSamplesEmptyPagedResource() {
    return legacySamplePagedResourcesAssembler.toEmptyResource(
        new PageImpl<>(new ArrayList<>(), new PageRequest(0, 50), 0), LegacySample.class, null);
  }

  private PagedResources getGroupsEmptyPagedResource() {
    return legacySamplePagedResourcesAssembler.toEmptyResource(
        new PageImpl<>(new ArrayList<>(), new PageRequest(0, 50), 0), LegacyGroup.class, null);
  }
