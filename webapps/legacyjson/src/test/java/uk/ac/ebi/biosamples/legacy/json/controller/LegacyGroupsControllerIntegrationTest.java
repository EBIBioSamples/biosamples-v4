...
public class LegacyGroupsControllerIntegrationTest {
  ...
  private PagedResources<Resource<Sample>> getTestPagedResourcesGroups(
  ...
  public void testRetrieveOfGroupByAccession() throws Exception {
    ...
    mockMvc
        .perform(get("/groups/SAMEG1").accept(HAL_JSON))
  ...
  public void testCamelcaseingOfCharacteristics() throws Exception {
    Sample group =
        new TestSample("SAMEG1")
            ...
            .withAttribute(Attribute.build("Sample_resource_Test", "Test"))
            .build();
    when(sampleRepository.findByAccession("SAMEG1")).thenReturn(Optional.of(group));
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.characteristics.sampleResourceTest[0].text").value("Test"));
  }
  ...
  public void testExternalLinksIsRootField() throws Exception {
    ...
    mockMvc
        .perform(get("/groups/{accession}", group.getAccession()).accept(HAL_JSON))
        .andExpect(
            jsonPath("$.externalReferences.*.name").value(containsInAnyOrder("other", "hPSCreg")))
  ...
  public void testGroupDisplayMSIinformations() throws Exception {
    ...
    mockMvc
        .perform(get("/groups/{accession}", sample.getAccession()).accept(HAL_JSON))
        .andExpect(
            jsonPath("$")
                .value(allOf(hasKey("contact"), hasKey("organization"), hasKey("publications"))))
            ...
            jsonPath("$.organization[0]")
                .value(
                    allOf(
                        ...
                        not(hasEntry("Address", "someAddress")))))
        .andExpect(
            jsonPath("$.publications[0]")
                .value(allOf(hasEntry("doi", "someDOI"), hasEntry("pubmed_id", "someID"))));
  }
  ...
  public void testGroupHasProperLinks() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$._links")
                .value(allOf(hasKey("self"), hasKey("group"), hasKey("relations"))));
  }
  ...
  public void testGroupsIndexReturnPagedResources() throws Exception {
    ...
    when(sampleRepository.findGroups(anyInt(), anyInt()))
        .thenReturn(getTestPagedResourcesGroups(10, group1, group2));

    mockMvc
        .perform(get("/groups").accept(HAL_JSON))
        .andExpect(status().isOk())
        .andExpect(content().contentType("application/hal+json;charset=UTF-8"))
        .andExpect(
            jsonPath("$").value(allOf(hasKey("_embedded"), hasKey("_links"), hasKey("page"))))
        .andExpect(
            jsonPath("$._embedded.groups.*.accession")
                .value(containsInAnyOrder("SAMEG1", "SAMEG2")));
  }
  ...
  public void testGroupsSearchHasExpectedLinks() throws Exception {
    mockMvc
        .perform(get("/groups/search").accept(HAL_JSON))
        .andExpect(
            jsonPath("$._links")
                .value(allOf(hasKey("self"), hasKey("findByKeywords"), hasKey("findByAccession"))));
  }
  ...
  public void testContactIsRootField() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.contact[0]")
                .value(allOf(hasKey("FirstName"), hasKey("Submitter"), hasKey("Email"))));
  }
  ...
  public void testOrganizationIsRootField() throws Exception {
    ...
    mockMvc
        .perform(get("/groups/{accession}", testGroup.getAccession()).accept(HAL_JSON))
