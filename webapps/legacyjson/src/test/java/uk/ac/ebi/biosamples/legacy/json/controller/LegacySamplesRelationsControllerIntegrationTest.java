...
public class LegacySamplesRelationsControllerIntegrationTest {
  ...
  private ResultActions getSamplesRelationsHAL(String accession) throws Exception {
    return mockMvc.perform(
        get("/samplesrelations/{accession}", accession).accept(MediaTypes.HAL_JSON_VALUE));
  }
  ...
  public void testSamplesRelationsHasSelfLink() throws Exception {
    ...
    getSamplesRelationsHAL("SAMN11").andExpect(jsonPath("$._links.self").exists());
  }
  ...
  public void testSamplesRelationsWithGroupIsReturned() throws Exception {
    ...
    Sample testSample =
        new TestSample("SAMEA111")
            .withRelationship(Relationship.build("SAMEG222", "has member", "SAMEA111"))
            .build();

    when(sampleRepository.findByAccession(testSample.getAccession()))
        .thenReturn(Optional.of(testSample));
    when(sampleRepository.findByAccession(testGroup.getAccession()))
        .thenReturn(Optional.of(testGroup));

    MvcResult result =
        getSamplesRelationsHAL("SAMEA111")
            .andExpect(jsonPath("$._links.groups.href").value(Matchers.endsWith("SAMEA111/groups")))
            .andReturn();

    String groupRelationsHref =
        JsonPath.parse(result.getResponse().getContentAsString()).read("$._links.groups.href");

    mockMvc
        .perform(get(groupRelationsHref).accept(MediaTypes.HAL_JSON))
        .andExpect(status().isOk())
        .andExpect(content().contentType("application/hal+json;charset=UTF-8"))
        .andExpect(jsonPath("$._embedded.groupsrelations[0].accession").value("SAMEG222"))
        .andExpect(jsonPath("$._links.self").exists())
        .andExpect(
            jsonPath("$._links.self.href").value(endsWith("samplesrelations/SAMEA111/groups")));
  }
  ...
  public void testAnotherSamplesRelationsWithGroupIsReturned() throws Exception {
    ...
    MvcResult result =
        getSamplesRelationsHAL("SAMEA222")
            .andExpect(jsonPath("$._links.groups.href").value(endsWith("SAMEA222/groups")))
            .andReturn();
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$._links.self").exists())
        .andExpect(
            jsonPath("$._links.self.href").value(endsWith("samplesrelations/SAMEA222/groups")));
  }

  @Test
  public void testDeriveToSamplesRelationIsReturned() throws Exception {
    Sample sample1 =
        new TestSample("SAME1")
            .withRelationship(Relationship.build("SAME2", "derivedFrom", "SAME1"))
            .build();
    Sample sample2 =
    ...
    String responseContent =
        getSamplesRelationsHAL(sample2.getAccession())
            .andExpect(status().isOk())
    ...
    mockMvc
        .perform(get(derivedFromHref).accept(MediaTypes.HAL_JSON))
        .andExpect(status().isOk())
        .andExpect(
            jsonPath("$._embedded.samplesrelations[0].accession").value(sample1.getAccession()));
  }
  ...
  public void testDerivedFromAndDerivedToWorkAsInverseSamplesRelations() throws Exception {
    Sample sample1 =
        new TestSample("SAME1")
            .withRelationship(Relationship.build("SAME1", "derivedTo", "SAME2"))
            .build();
    Sample sample2 =
        new TestSample("SAME2")
            .withRelationship(Relationship.build("SAME1", "derivedTo", "SAME2"))
    ...
    String samplesRelationsResponseContent =
        getSamplesRelationsHAL(sample2.getAccession())
            .andExpect(status().isOk())
            .andExpect(jsonPath("$._links.derivedFrom.href").value(endsWith("SAME2/derivedFrom")))
            .andReturn()
    ...
    String derivedFromHref =
        JsonPath.parse(samplesRelationsResponseContent).read("$._links.derivedFrom.href");

    String derivedToSamplesResponseContent =
        mockMvc
            .perform(get(derivedFromHref).accept(MediaTypes.HAL_JSON))
            .andExpect(status().isOk())
            .andExpect(
    ...
    String derivedToHref =
        JsonPath.parse(derivedToSamplesResponseContent)
            .read("$._embedded.samplesrelations[0]._links.derivedTo.href");

    mockMvc
        .perform(get(derivedToHref).accept(MediaTypes.HAL_JSON))
        .andExpect(status().isOk())
        .andExpect(
            jsonPath("$._embedded.samplesrelations[0].accession").value(sample2.getAccession()));
  }

  @Test
  public void testSamplesWithNoAssociatedSamplesRelationReturnEmptyResourcesObject()
      throws Exception {
    Sample testSample = new TestSample("SAMEA1").build();
    when(sampleRepository.findByAccession(testSample.getAccession()))
        .thenReturn(Optional.of(testSample));

    String samplesRelationsContent =
        getSamplesRelationsHAL(testSample.getAccession())
            .andReturn()
            .getResponse()
            .getContentAsString();
    ...
    String derivedFromHref =
        JsonPath.parse(samplesRelationsContent).read("$._links.derivedFrom.href");
    mockMvc
        .perform(get(derivedFromHref).accept(MediaTypes.HAL_JSON))
  ...
  public void testRetrieveUnknownSamplesRelationsFromLegacyApiThrowsError() throws Exception {
    ...
    mockMvc
        .perform(get("/samplesrelations/SAME2/unknownRelation").accept(MediaTypes.HAL_JSON))
  ...
  public void testRetrieveAllSamplesRelationsReturnPagedResource() throws Exception {
    ...
    mockMvc
        .perform(get("/samplesrelations").accept(MediaTypes.HAL_JSON))
        .andExpect(status().isOk())
        .andExpect(content().contentType("application/hal+json;charset=UTF-8"))
  ...
  @Test
  public void testRetrieveAllSamplesRelationsOnePerPageEffectivelyReturnOneRelationsPerPage()
      throws Exception {
    ...
    mockMvc
        .perform(get("/samplesrelations?page=0&size=1").accept(MediaTypes.HAL_JSON))
        .andExpect(status().isOk())
  ...
  public void testAllSamplesRelationsHasLinkToSearch() throws Exception {
    ...
    String jsonContent =
        mockMvc
            .perform(get("/samplesrelations").accept(MediaTypes.HAL_JSON))
            .andExpect(jsonPath("$._links.search.href").value(endsWith("/samplesrelations/search")))
            .andReturn()
            .getResponse()
            .getContentAsString();
    ...
    mockMvc
        .perform(get(searchEndpoint).accept(MediaTypes.HAL_JSON))
        .andExpect(status().isOk())
        .andExpect(content().contentType("application/hal+json;charset=UTF-8"))
  ...
  public void testSamplesRelationsSearchHasLinkFindOneByAccession() throws Exception {
    ...
    mockMvc
        .perform(get("/samplesrelations/search").accept(MediaTypes.HAL_JSON))
        .andExpect(status().isOk())
        .andExpect(content().contentType("application/hal+json;charset=UTF-8"))
        .andExpect(jsonPath("$._links.findOneByAccession").exists())
        .andExpect(jsonPath("$._links.findOneByAccession.templated").isBoolean())
  ...
  public void testSamplesRelationsFindOneByAccessionReturnTheCorrectSample() throws Exception {
    ...
    UriTemplate uriTemplate =
        new UriTemplate(
            JsonPath.parse(searchLinksContent).read("$._links.findOneByAccession.href"));

    mockMvc
        .perform(get(uriTemplate.expand("SAMEA1")).accept(MediaTypes.HAL_JSON))
        .andExpect(status().isOk())
        .andExpect(content().contentType("application/hal+json;charset=UTF-8"))
  ...
  @Test
  public void testSamplesRelationsFindOneByAccessionReturn404ForNonExistingSample()
      throws Exception {
    ...
    mockMvc
        .perform(get(uriTemplate.expand("SAMD2")).accept(MediaTypes.HAL_JSON))
        .andExpect(status().isNotFound());
  }

  /**
   * Check that all the relations' href exposed by the sample relations endpoint are returning a 200
   * code
   *
   * @throws Exception
   */
  ...
  public void allProvidedRelationsLinksShouldReturn200Status() throws Exception {
    ...
    String contentAsString =
        mockMvc
            .perform(
                get("/samplesrelations/" + testSample.getAccession()).accept(MediaTypes.HAL_JSON))
            .andExpect(status().isOk())
            .andReturn()
            .getResponse()
            .getContentAsString();
  ...
  public void detailsLinkPointToSamplePage() throws Exception {
    ...
    mockMvc
        .perform(get(detailHref).accept(MediaTypes.HAL_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.accession").value(testSample.getAccession()));
  }
