...
public class SearchFilterMapper {
  ...
  private static void getSearchFilters(Set<uk.ac.ebi.biosamples.core.model.filter.Filter> filters, List<Filter> grpcFilters) {
    ...
    for (uk.ac.ebi.biosamples.core.model.filter.Filter filter : filters) {
      ...
      } else if (filter instanceof uk.ac.ebi.biosamples.core.model.filter.ExternalReferenceDataFilter f) {
  ...
  private static void getAuthSearchFilter(AuthenticationFilter f, List<Filter> grpcFilters) {
    f.getContent().ifPresent(auth -> { // todo domain
      grpcFilters.add(Filter.newBuilder().setWebin(WebinIdFilter.newBuilder().setWebinId(auth)).build());
    });
  }

  private static void getNameSearchFilter(uk.ac.ebi.biosamples.core.model.filter.NameFilter f, List<Filter> grpcFilters) {
    f.getContent().ifPresent(name ->
        grpcFilters.add(Filter.newBuilder().setName(NameFilter.newBuilder().setName(name)).build()));
  }

  private static void getAccessionSearchFilter(uk.ac.ebi.biosamples.core.model.filter.AccessionFilter f, List<Filter> grpcFilters) {
    f.getContent().ifPresent(accession ->
        grpcFilters.add(Filter.newBuilder().setAccession(AccessionFilter.newBuilder().setAccession(accession)).build()));
  }

  private static void getExternalReferenceSearchFilter(ExternalReferenceDataFilter f, List<Filter> grpcFilters) {
  ...
  private static void getInverseRelationshipSearchFilter(InverseRelationFilter f, List<Filter> grpcFilters) {
  ...
  private static void getDateRangeSearchFilter(uk.ac.ebi.biosamples.core.model.filter.DateRangeFilter f, List<Filter> grpcFilters) {
    f.getContent().ifPresent(dateRange -> {
      ...
      grpcFilters.add(Filter.newBuilder().setDateRange(
  ...
  private static void getAttributeSearchFilter(uk.ac.ebi.biosamples.core.model.filter.AttributeFilter filter,
                                               Map<String, Filter.Builder> filterMap) {
    Filter.Builder fb = filterMap.getOrDefault(filter.getLabel(), Filter.newBuilder()
