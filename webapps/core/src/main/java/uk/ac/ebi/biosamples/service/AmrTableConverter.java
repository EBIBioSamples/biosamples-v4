...
public class AmrTableConverter implements Converter<AMRTable, List<List<String>>> {
  ...
  public List<List<String>> convert(AMRTable amrTable) {
    ...
    //
    //        List<String> tableHeaderSorted = Arrays.asList("Species", "Antibiotic Name", "Ast
    // Standard", "Breakpoint Version",
    //                "Laboratory Typing Method", "Measurement Value", "Measurement Units",
    // "Measurement Sign",
  ...
  public class HtmlAmrTable {
    ...
    public List<List<String>> getCompleteTable() {
      List<String> headerList =
          Arrays.stream(headers).filter(this.columnElementCount::get).collect(Collectors.toList());
