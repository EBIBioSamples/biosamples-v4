...
public class SamplePageService {
	...
	public Page<Sample> getSamplesByText(String text, Collection<Filter> filters, Collection<String> domains, Pageable pageable, String curationRepo) {
	...
	public CursorArrayList<Sample> getSamplesByText(String text, Collection<Filter> filters,
													Collection<String> domains, String cursorMark, int size, String curationRepo) {
	...
	private List<Sample> collectSampleFutures(List<Future<Optional<Sample>>> listFutureSample) {
		return listFutureSample.stream().map(ss -> {
			try {
				return ss.get().get();
			} catch (InterruptedException e) {
				throw new RuntimeException(e);
