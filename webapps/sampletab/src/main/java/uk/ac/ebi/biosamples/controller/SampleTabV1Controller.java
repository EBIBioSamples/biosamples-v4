...
public class SampleTabV1Controller {
  ...
  @Autowired private SampleTabService sampleTabService;
  @Autowired private ApiKeyService apiKeyService;
  @Autowired private SampleTabMultipartFileConverter sampleTabFileConverter;
  ...
  /**
   * Temporary solution to support direct submission of SampleTabs as files instead of JSON String
   * matrix This is not ideal, and probably we should handle this differently by updating the
   * SampleTab parser instead of converting the file into a List<List<String>> to reuse the code
  ...
  public @ResponseBody Outcome doAccession(
      @RequestBody SampleTabRequest request, @RequestParam(value = "apikey") String apiKey) {
    // handle APIkey
    if (apiKey == null) {
      Outcome outcome =
          getErrorOutcome(
              "API key not present",
              "API key not present. Contact biosamples@ebi.ac.uk for more information.");
      return outcome;
    }
    ...
    if (!domain.isPresent()) {
      Outcome outcome =
          getErrorOutcome(
              ...
              "API key "
                  + apiKey
                  + " was not recognized as a valid API key. Contact biosamples@ebi.ac.uk for more information.");
      return outcome;
    }
    ...
    if (outcome.getErrors().size() > 0) {
      ...
      try {
        outcome.sampledata =
            sampleTabService.accessionSampleTab(
                outcome.sampledata, "self." + domain.get(), null, true);
      } catch (SampleTabException e) {
        return getErrorOutcome(
            "Unable to accession",
            e.getMessage() + " Contact biosamples@ebi.ac.uk for more information.");
      }
      return outcome;
    }
  }
  ...
  public @ResponseBody Outcome accessionFile(
      @RequestParam("file") MultipartFile sampleTabFile,
  ...
  public @ResponseBody Outcome doSubmission(
      @RequestBody SampleTabRequest request, @RequestParam(value = "apikey") String apiKey) {
    // handle APIkey
    if (apiKey == null) {
      Outcome outcome =
    ...
    if (outcome.getErrors().size() > 0) {
      ...
      try {
        outcome.sampledata =
            sampleTabService.saveSampleTab(
                outcome.sampledata, "self." + domain.get(), isSuperuser, true, true);
      } catch (SampleTabException e) {
  ...
  public @ResponseBody Outcome submitFile(
      @RequestParam("file") MultipartFile sampleTabFile, @RequestParam("apikey") String apiKey) {
  ...
  private Outcome parse(SampleTabRequest request) {
    ...
    parser.addErrorItemListener(
        new ErrorItemListener() {
  ...
  public static class SampleTabRequest {
    ...
    /**
     * Default constructor to allow deserialization of JSON into a request bean: present to allow
     * Jackson/spring to construct a request bean from POST requests properly.
     */
