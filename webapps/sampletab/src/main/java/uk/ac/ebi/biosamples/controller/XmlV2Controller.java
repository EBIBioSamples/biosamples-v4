...
public class XmlV2Controller {
  ...
  @PostMapping(
      value = "/source/{source}/sample",
      produces = MediaType.TEXT_PLAIN_VALUE,
      consumes = {MediaType.APPLICATION_XML_VALUE})
  public ResponseEntity<String> saveSourceSampleNew(
  ...
  @PostMapping(value = "/source/{source}/sample/{sourceid}", produces = MediaType.TEXT_PLAIN_VALUE)
  public @ResponseBody ResponseEntity<String> saveSourceSample(
      @PathVariable String source, @PathVariable String sourceid, @RequestParam String apikey) {
    ...
    if (!apiKeyService.canKeyOwnerEditSource(keyOwner.get(), source)) {
      return new ResponseEntity<String>(
          "That API key is not permitted for that source", HttpStatus.FORBIDDEN);
    }
    ...
    if (bioSamplesClient.fetchSampleResourceAll(null, filterList).iterator().hasNext()) {
      return new ResponseEntity<String>(
          "POST must be a new submission, use PUT for updates", HttpStatus.BAD_REQUEST);
    }
  ...
  @PostMapping(
      value = "/source/{source}/sample/{sourceid}",
      produces = MediaType.TEXT_PLAIN_VALUE,
      consumes = {MediaType.APPLICATION_XML_VALUE})
  public @ResponseBody ResponseEntity<String> saveSourceSample(
      @PathVariable String source,
    ...
    if (sample != null && sample.getAccession() != null) {
      return ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED)
          .body("Cannot POST a sample with an existing accession, use PUT for updates.");
    }
  ...
  @PutMapping(
      ...
      produces = MediaType.TEXT_PLAIN_VALUE,
      consumes = {MediaType.APPLICATION_XML_VALUE})
  public @ResponseBody ResponseEntity<String> saveUpdate(
      @PathVariable String source,
      @PathVariable String sourceid,
      @RequestParam String apikey,
    ...
    if (!sourceid.matches("SAM[NED]A?[0-9]+")) {
      return new ResponseEntity<String>(
          "Only implemented for BioSample accessions", HttpStatus.FORBIDDEN);
    }

    // reject if has no accession
    if (sample != null && sample.getAccession() == null) {
      return ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED)
          .body("Cannot PUT a sample without an existing accession, use POST for new samples.");
    }
    ...
    // if no accession, try and find any samples that already exist with this sourceid and use
    // their
    // accession
    // NB we don't validate that a sample *must* have this sourceid
    // FIXME: here sample could be null and generate NullPointerException, check before for
    // rejection on no accession provided
    if (sample.getAccession() == null) {
      ...
      if (!it.hasNext()) {
        return new ResponseEntity<String>(
            "PUT must be an update, use POST for new submissions", HttpStatus.BAD_REQUEST);
      } else {
        Resource<Sample> first = it.next();
        if (it.hasNext()) {
          // error multiple accessions
          return new ResponseEntity<String>(
              "Multiple samples with name " + sourceid + " in domain " + domain.get(),
              HttpStatus.BAD_REQUEST);
        } else {
          sample =
              Sample.build(
                  sample.getName(),
                  first.getContent().getAccession(),
                  null,
  ...
  @GetMapping(value = "/source/{source}/sample/{sourceid}", produces = MediaType.TEXT_PLAIN_VALUE)
  public @ResponseBody ResponseEntity<String> getAccessionOfSample(
      @PathVariable String source, @PathVariable String sourceid, @RequestParam String apikey) {
    log.warn("ACCESSING DEPRECATED API at XmlV2Controller /source/{source}/sample/{sourceid}");
    ...
    if (it.hasNext()) {
      ...
      if (it.hasNext()) {
        return new ResponseEntity<String>(
            "Multiple samples with sourceid " + sourceid, HttpStatus.BAD_REQUEST);
      } else {
  ...
  */
  ...
  @PostMapping(
      value = "/source/{source}/group",
      produces = "text/plain",
      consumes = "application/xml")
  public ResponseEntity<String> saveSourceGroupNew(
      @PathVariable String source,
      @RequestParam String apikey,
  ...
  @PostMapping(
      value = "/source/{source}/group/{sourceid}",
      produces = "text/plain",
      consumes = "application/xml")
  public @ResponseBody ResponseEntity<String> saveSourceGroup(
      @PathVariable String source,
      @PathVariable String sourceid,
      @RequestParam String apikey,
      @RequestBody(required = false) Sample group)
    ...
    if (group != null && group.getAccession() != null) {
      return ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED)
          .body("Cannot POST a group with an existing accession, use PUT for updates.");
    }
  ...
  @PutMapping(
      ...
      produces = "text/plain",
      consumes = "application/xml")
  public @ResponseBody ResponseEntity<String> saveGroupUpdate(
      @PathVariable String source,
      @PathVariable String sourceid,
      @RequestParam String apikey,
    ...
    if (group != null && group.getAccession() == null) {
      return ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED)
          .body("Cannot PUT a group without an existing accession, use POST for new samples.");
    }
