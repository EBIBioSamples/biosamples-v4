...
public class SampleGroupSubmissionService {
    ...
    public SampleGroupSubmissionService(RestOperations restOperations, Traverson traverson, ExecutorService executor) {
     ...
     * @return sample wrapped in resource
     */
    public Future<Resource<Sample>> submitAsync(Sample sample, String jwt) throws RestClientException {
    ...
    private class SubmitCallable implements Callable<Resource<Sample>> {
        ...
        public Resource<Sample> call() throws Exception {
            ...
            RequestEntity.BodyBuilder bodyBuilder = update ? RequestEntity.put(uri) : RequestEntity.post(uri);
            ...
            try {
                ...
                LOGGER.error("Failed to persist sample group, uri: {}, body: {}, response: {}", uri, sample, e.getResponseBodyAsString());
                throw e;
            }
        ...
        private URI getSamplePersistURI(Link sampleLink, String accession) {
            ...
            if (accession == null) {
                ...
                uriComponentsBuilder = UriComponentsBuilder.fromUriString(sampleLink.getHref() + "/" + accession);
            }
