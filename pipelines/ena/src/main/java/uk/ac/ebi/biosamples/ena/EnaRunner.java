...
import uk.ac.ebi.biosamples.ena.service.amr.AmrDataLoaderService;
...
public class EnaRunner implements ApplicationRunner {
    ...
    private void importEraBsdAuthoritySamples(final LocalDate fromDate, final LocalDate toDate) throws Exception {
        log.info("Handling BioSamples Authority Samples");

        if (pipelinesProperties.getThreadCount() == 0) {
            final EraRowBsdSamplesCallbackHandler eraRowBsdSamplesCallbackHandler = new EraRowBsdSamplesCallbackHandler(null, enaCallableFactory, futures);

            eraProDao.doSampleCallbackBsdAuthoritySamples(fromDate, toDate, eraRowBsdSamplesCallbackHandler);
        } else {
            try (final AdaptiveThreadPoolExecutor executorService = AdaptiveThreadPoolExecutor.create(100, 10000, false,
                    pipelinesProperties.getThreadCount(), pipelinesProperties.getThreadCountMax())) {
                final EraRowBsdSamplesCallbackHandler eraRowBsdSamplesCallbackHandler = new EraRowBsdSamplesCallbackHandler(executorService,
    ...
    private void importEraSamples(final LocalDate fromDate, final LocalDate toDate, final Map<String, Set<AbstractData>> sampleToAmrMap)
            throws Exception {
        log.info("Handling ENA and NCBI Samples");

        if (pipelinesProperties.getThreadCount() == 0) {
            final EraRowCallbackHandler eraRowCallbackHandler = new EraRowCallbackHandler(null, enaCallableFactory, futures, sampleToAmrMap);

            eraProDao.doSampleCallback(fromDate, toDate, eraRowCallbackHandler);

            final NcbiRowCallbackHandler ncbiRowCallbackHandler = new NcbiRowCallbackHandler(null, ncbiCallableFactory,
                    futures);
            ...
            try (final AdaptiveThreadPoolExecutor executorService = AdaptiveThreadPoolExecutor.create(100, 10000, false,
                ...
                final EraRowCallbackHandler eraRowCallbackHandler = new EraRowCallbackHandler(executorService,
                        enaCallableFactory, futures, sampleToAmrMap);

                eraProDao.doSampleCallback(fromDate, toDate, eraRowCallbackHandler);

                final NcbiRowCallbackHandler ncbiRowCallbackHandler = new NcbiRowCallbackHandler(executorService,
    ...
    private static class EraRowCallbackHandler implements RowCallbackHandler {
        ...
        public EraRowCallbackHandler(final AdaptiveThreadPoolExecutor executorService, final EnaCallableFactory enaCallableFactory,
                                     final Map<String, Future<Void>> futures, final Map<String, Set<AbstractData>> sampleToAmrMap) {
            this.executorService = executorService;
            this.enaCallableFactory = enaCallableFactory;
    ...
    private static class NcbiRowCallbackHandler implements RowCallbackHandler {
        ...
        public NcbiRowCallbackHandler(final AdaptiveThreadPoolExecutor executorService,
                                      final NcbiCurationCallableFactory ncbiCallableFactory, final Map<String, Future<Void>> futures) {
            this.executorService = executorService;
    ...
    private static class EraRowBsdSamplesCallbackHandler implements RowCallbackHandler {
        ...
        public EraRowBsdSamplesCallbackHandler(final AdaptiveThreadPoolExecutor executorService, final EnaCallableFactory enaCallableFactory, final Map<String, Future<Void>> futures) {
            this.executorService = executorService;
            this.enaCallableFactory = enaCallableFactory;
            this.futures = futures;
        }
