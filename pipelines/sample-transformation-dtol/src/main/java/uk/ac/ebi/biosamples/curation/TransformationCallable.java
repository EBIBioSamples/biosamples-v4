...
public class TransformationCallable implements Callable<PipelineResult> {
    ...
    public PipelineResult call() {
        ...
        Optional<Resource<Sample>> optionalSampleResource = bioSamplesClient.fetchSampleResource(sample.getAccession(), Optional.of(new ArrayList<>()));
        if (optionalSampleResource.isPresent()) {
            Sample uncuratedSample = optionalSampleResource.get().getContent();
            Optional<Attribute> optionalRelAttribute = uncuratedSample.getAttributes().stream()
            ...
            if (optionalRelAttribute.isPresent()) {
                ...
                uncuratedSample.getRelationships().add(createDerivedRelationship(uncuratedSample.getAccession(), attribute.getValue()));
                bioSamplesClient.persistSampleResource(uncuratedSample);
                LOG.info("Copied derived from relationship from attribute {} -> {}", uncuratedSample.getAccession(), attribute.getValue());
                modifiedRecords++;
            } else {
