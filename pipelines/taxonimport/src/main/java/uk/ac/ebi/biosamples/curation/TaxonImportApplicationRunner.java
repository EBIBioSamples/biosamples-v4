...
public class TaxonImportApplicationRunner implements ApplicationRunner {
  ...
  public TaxonImportApplicationRunner(final BioSamplesClient bioSamplesClient,
                                      final AnalyticsService analyticsService, ObjectMapper objectMapper) {
  ...
  public void run(final ApplicationArguments args) throws Exception {
    ...
    for (TaxonEntry entry : taxonEntries) {
      ...
      Optional<EntityModel<Sample>> optionalSample = bioSamplesClient.fetchSampleResource(entry.getBioSampleAccession(), Optional.of(Collections.singletonList("")));
      if (optionalSample.isPresent()) {
        Sample sample = optionalSample.get().getContent();
        Long oldTaxId = sample.getTaxId();
        Optional<Attribute> oldOrganism = sample.getCharacteristics().stream().filter(c -> c.getType().equalsIgnoreCase("organism")).findFirst();
        ...
        if (oldOrganism.isPresent()) {
          ...
          LOG.info("Curating sample " + entry.getBioSampleAccession()
              + " : taxId = " + oldTaxId + " -> " + entry.getTaxId()
              + " : organism = " + oldOrganism.get().getValue() + " -> " + entry.getBioSampleTaxName());
        } else {
          LOG.info("Curating sample " + entry.getBioSampleAccession()
              + " : taxId = " + oldTaxId + " : organism = " + entry.getBioSampleTaxName());
        }
    ...
    final PipelineAnalytics pipelineAnalytics =
        new PipelineAnalytics(
            "taxonimport", startTime, endTime, taxonEntries.size(), pipelineFutureCallback.getTotalCount());
  ...
  private List<TaxonEntry> loadTaxonFile(final String filePath) {
    ...
    try {
      ...
      throw new RuntimeException("Failed to process the json file. Pipeline failed to run. Exiting the pipeline.");
    }
  ...
  private String getFileNameFromArgs(final ApplicationArguments args) {
    ...
    if (args.getOptionNames().contains("file")) {
      ...
      taxonFile = "/lts/production/tburdett/ena/taxon_name_change/taxname_change_20230414_1551_SAMN.json";
    }
