...
public class NcbiAmrConvertionTests {
  ...
  public void given_amr_data_it_produces_sample_with_structured_data() {
    Set<StructuredDataTable> structuredData =
        sampleConversionService.convertNcbiXmlElementToStructuredData(getAmrSample(), new HashSet<>());
    assertEquals("Should contain only 1 AMR element", 1, structuredData.size());
    assertEquals("Sample should contain AMR data", structuredData.iterator().next().getType(),
                 StructuredDataType.AMR.name());
    assertEquals("Should contain only 1 row", 1, structuredData.iterator().next().getContent().size());
  }
