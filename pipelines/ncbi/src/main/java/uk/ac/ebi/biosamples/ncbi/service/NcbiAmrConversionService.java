...
public class NcbiAmrConversionService {
  public Set<Map<String, StructuredDataEntry>> convertStructuredTable(
      final Element amrTableElement, final String organism) throws AmrParsingException {
    final List<String> fields =
        XmlPathBuilder.of(amrTableElement).path("Header").elements("Cell").stream()
            .map(Element::getText)
            .collect(Collectors.toList());

    final Set<Map<String, StructuredDataEntry>> dataEntrySet = new HashSet<>();
    for (final Element tableRow : XmlPathBuilder.of(amrTableElement).path("Body").elements("Row")) {
  ...
  private Map<String, StructuredDataEntry> getStructuredDataRow(
      final Element amrRowElement, final List<String> fields, final String organism)
    ...
    final Map<String, StructuredDataEntry> dataEntryMap = new HashMap<>();
  ...
  private Optional<String> getFieldIfAvailable(
      final List<String> values, final List<String> fields, final String fieldToFind) {
    String fieldValue = null;

    if (fields.contains(fieldToFind)) {
      final int fieldIndex = fields.indexOf(fieldToFind);
  ...
  public static class AmrParsingException extends ParseException {
    AmrParsingException(final String s) {
      super(s, -1);
    }
  }
}
