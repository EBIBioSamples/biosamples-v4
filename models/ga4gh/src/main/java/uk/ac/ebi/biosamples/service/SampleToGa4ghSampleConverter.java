...
public class SampleToGa4ghSampleConverter implements Converter<Sample, Ga4ghSample> {
  ...
  @Autowired OLSDataRetriever olsRetriever;

  @Autowired
  public SampleToGa4ghSampleConverter(
      Ga4ghSample ga4ghSample,
      GeoLocationDataHelper helper,
      HttpOlsUrlResolutionService httpOlsUrlResolutionService) {
  ...
  /**
   * Maps characteristics from Biosamples sample to GA4GH sample. Sorts data to: biocharacteristics,
   * attributes, geolocation info, age info,description, dataset_id.
   *
   * @param rawSample sample retrieved from Biosamples
   */
    ...
    for (Attribute attribute : characteristics) {
      ...
      if (type.equals("age") || type.equals("age_years") || type.equals("age(years)")) {
        ...
        if (attribute.getValue() != null
            && httpOlsUrlResolutionService.getIriOls(attribute.getIri()) != null) {
   ...
   * contacts, released and updated dates, contacts, domain, external_references, organizations,
   * publications.
   *
   * @param rawSample sample retreived from Biosamples
   */
  private void mapRemainingData(Sample rawSample) {
    ...
    if (!rawSample.getContacts().isEmpty()) {
      ga4ghSample.addAttributeList(
          "contacts", convertObjectsToAttributeValues(rawSample.getContacts()));
    }
    if (!rawSample.getExternalReferences().isEmpty()) {
      ga4ghSample.addAttributeList(
          "external_references",
          convertObjectsToAttributeValues(rawSample.getExternalReferences()));
    }
    if (!rawSample.getOrganizations().isEmpty()) {
      ga4ghSample.addAttributeList(
          "organizations", convertObjectsToAttributeValues(rawSample.getOrganizations()));
    }
    if (!rawSample.getPublications().isEmpty()) {
      ga4ghSample.addAttributeList(
          "publications", convertObjectsToAttributeValues(rawSample.getPublications()));
    }
  }
  ...
  /**
   * Convert java objects to pairs of names and values of fields. This will be used in convertation
   * that to ga4gh format. From this pairs will be created AttributeValues that will be wrapped by
   * Attributes.
   *
