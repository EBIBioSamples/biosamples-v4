...
public class LegacyJSONDeserializationTest {
  ...
  @Autowired JSONSampleToSampleConverter sampleConverter;
  ...
  public void testConvertDescriptionToAttribute() {
    assertThat(legacySample.getAttributes())
        .contains(Attribute.build("description", "This is a simple description"));
  }
  ...
  public void testContainsCertainAttributes() {
    assertThat(
            legacySample.getAttributes().stream()
  ...
  public void testAttributeContainsOntologyTerms() {
    List<Attribute> organismAttribute =
        legacySample.getAttributes().stream()
            .filter(att -> att.getType().equals("organism"))
            .collect(Collectors.toList());
    assertThat(organismAttribute).hasSize(1);
    assertThat(organismAttribute.get(0).getIri())
        .containsOnlyOnce("http://purl.obolibrary.org/obo/NCBITaxon_9606");
  }

  @Test
  public void testConvertContact() {
    assertThat(legacySample.getContacts())
        .containsOnly(new Contact.Builder().name("Leo Zeef").build());
  }

  @Test
  public void testConvertOrganization() {
    assertThat(legacySample.getOrganizations())
        .containsOnly(
            new Organization.Builder().name("Vanderbilt University").role("submitter").build());
  }

  @Test
  public void testConvertPublications() {
    assertThat(legacySample.getPublications())
        .containsOnly(
            new Publication.Builder()
                .doi("doi:10.1091/mbc.E11-05-0426")
                .pubmed_id("21680712")
                .build());
  }
  ...
  public void testConvertEmbeddedExternalReferences() {
    assertThat(legacySample.getExternalReferences())
        .contains(ExternalReference.build("http://www.ebi.ac.uk/ena/data/view/ERP006121"));
  }

  @Test
  public void testConvertGroupSamplesIntoRelationships() {
    legacyGroup = sampleConverter.convert(groupJson);
    assertThat(legacyGroup.getRelationships())
        .contains(Relationship.build("SAMEG316628", "has member", "SAMEA4562408"));
  }
