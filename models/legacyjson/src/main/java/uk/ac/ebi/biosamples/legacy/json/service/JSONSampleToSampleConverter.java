...
public class JSONSampleToSampleConverter implements Converter<String, Sample> {
  ...
  public Sample convert(String source) {
    ...
    SortedSet<ExternalReference> embeddedExternalReferences =
        getEmbeddedExternalReferences(jsonDoc);

    createDate = createDate == null || createDate.trim().isEmpty() ? updateDate : createDate;

    Sample.Builder sampleBuilder =
        new Sample.Builder(sampleName, accession)
      ...
      if (testValue instanceof String) {
        ...
        if (dois != null && pubmed_ids != null) {
          ...
          throw new RuntimeException(
              "Unexpected format for publications " + serializedPublications);
        }
      } else {
  ...
  private SortedSet<Attribute> getSubmissionInformations(DocumentContext json) {
    ...
    if (subAccession != null) submissionData.add(Attribute.build("submission_acc", subAccession));

    if (subTitle != null) submissionData.add(Attribute.build("submission_title", subTitle));
      ...
      if (testValue instanceof String) {
        ...
        for (Map<String, Object> extRef : externalReferenceList) {
          ...
          SortedSet<String> duoCodes =
              extRef.get("duo") != null
                  ? new TreeSet<>((List<String>) extRef.get("duo"))
                  : new TreeSet<>();
