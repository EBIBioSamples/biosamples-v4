...
public class MessageBuffer<S> {
	private Logger log = LoggerFactory.getLogger(this.getClass());

	private final CrudRepository<S,?> repository;	
	...
	public MessageBuffer(@Autowired CrudRepository<S,?> repository) {
	...
	public void checkQueueStatus() {
		...
		if (remaining <= 0.1*QUEUE_SIZE
			...
			try {
				...
				//send everything as a single commit
