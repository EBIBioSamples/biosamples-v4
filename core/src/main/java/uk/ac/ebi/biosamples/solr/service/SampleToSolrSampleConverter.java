...
import uk.ac.ebi.biosamples.core.service.ExternalReferenceService;
import uk.ac.ebi.biosamples.core.service.SampleRelationshipUtils;
...
public class SampleToSolrSampleConverter implements Converter<Sample, SolrSample> {
  ...
  public SolrSample convert(final Sample sample) {
    ...
    if (sample.getCharacteristics() != null && !sample.getCharacteristics().isEmpty()) {
      for (final Attribute attr : sample.getCharacteristics()) {
        final String key = SolrFieldService.encodeFieldName(attr.getType());

        String value = attr.getValue();
        // if it's longer than 255 characters, don't add it to solr
        //  cant index long things well, and It's probably not useful for search
        if (value.length() > 255) {
          continue;
        }
        ...
        if (attr.getIri().isEmpty()) {
          ...
          final List<String> iris = attr.getIri().stream().map(this::getOntologyFromIri).toList();

          attributeIris.get(key).addAll(iris);
          keywords.addAll(iris.stream().map(String::toLowerCase).toList());
        }
