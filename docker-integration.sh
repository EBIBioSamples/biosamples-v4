...
do
  ...
  docker-compose run --rm --service-ports biosamples-integration java -jar integration-4.0.4.jar --phase=$X $ARGS $@
  docker-compose run --rm --service-ports biosamples-agents-solr java -jar agents-solr-4.0.4.jar --biosamples.agent.solr.stayalive=false --biosamples.agent.solr.queuetime=500
done
