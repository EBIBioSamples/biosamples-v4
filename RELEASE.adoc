...
* Sample JSON now contains a taxId field at the top level
* Ontology terms now resolve to the defining ontology when they are available in multiple ontologies
* Requests for a sample now contain a computed ETag error to identify changes
* When requesting a private sample an explaination message is not provided as well as the 403 error code
* The search UI now contains a clear filers button
