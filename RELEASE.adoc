...
Private sample submitted using Webin Authentication can now be searched using the GET API, it covers:

Single private sample search using accession
Filtered search result containing only private samples
Filtered search results containing a mix of private and public samples
...
Publications, contacts and organizations can now be added to sample metadata for submission using the drag’n’drop uploader. For more details please refer - https://www.ebi.ac.uk/biosamples/docs/cookbook/upload_files.html

*Generic structured data submission model*

We have refactored structured data API to accept generic data structures. Previous structured data API only accepted a predefined set of types and hence each new type required code changes. Furthermore, structured data is completely decoupled from the sample in the persistence layer to fully reflect different ownership models they have. More details about the API can be found in our documentation here https://www.ebi.ac.uk/biosamples/docs/references/api/submit#_submit_structured_data_to_sample.

=== Bug Fixes
BioSamples API documentation has been fixed to include the http requests and http response snippets

=== New V2 endpoints
Improved V2 submission and accession endpoints will be deployed with this release. This will be first integrated with ENA for accessioning. The endpoints will be monitored for performance. Depending on the reliability metrics the endpoints will be documented and made available for users. Expected date of general availability is December 10, 2021, if performance results are as expected. Target availability is 99.5%.
