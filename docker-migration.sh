...
ls $SUBS_HOME/*/sampletab.pre.txt | xargs -n 1 -I {} curl -X POST -H "Content-Type: application/text" --data-binary @{} http://localhost:8082/biosamples/beta/sampletab/v4
echo "Imported SampleTab submissions"

#import from ena
echo "Importing from ENA"
time java -jar pipelines/ena/target/pipelines-ena-4.0.0-SNAPSHOT.jar --biosamples.submissionuri=http://localhost:8081/biosamples/beta  --logging.file=./docker/logs/pipelines-ena.log
...
time java -jar pipelines/ncbi/target/pipelines-ncbi-4.0.0-SNAPSHOT.jar --biosamples.submissionuri=http://localhost:8081/biosamples/beta --biosamples.ncbi.file=/tmp/biosample_set.xml.gz --logging.file=./docker/logs/pipelines-ncbi.log
