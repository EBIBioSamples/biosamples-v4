...
public class RestFilterIntegration extends AbstractIntegration {
    ...
    protected void phaseOne() {
        ...
        if (optionalSample.isPresent()) {
            throw new IntegrationTestFailException("RestFilterIntegration test sample should not be available during phase 1", Phase.ONE);
        }

        Resource<Sample> resource = client.persistSampleResource(testSample1);
        testSample1 = Sample.Builder.fromSample(testSample1).withAccession(resource.getContent().getAccession()).build();
        if (!testSample1.equals(resource.getContent())) {
            throw new IntegrationTestFailException("Expected response (" + resource.getContent() + ") to equal submission (" + testSample1 + ")", Phase.ONE);
        }

        resource = client.persistSampleResource(testSample2);
        testSample2 = Sample.Builder.fromSample(testSample2).withAccession(resource.getContent().getAccession()).build();
        if (!testSample2.equals(resource.getContent())) {
            throw new IntegrationTestFailException("Expected response (" + resource.getContent() + ") to equal submission (" + testSample2 + ")", Phase.ONE);
        }

        resource = client.persistSampleResource(testSample3);
        testSample3 = Sample.Builder.fromSample(testSample3).withAccession(resource.getContent().getAccession()).build();
        if (!testSample3.equals(resource.getContent())) {
            throw new IntegrationTestFailException("Expected response (" + resource.getContent() + ") to equal submission (" + testSample3 + ")", Phase.ONE);
        }
    }
    ...
    protected void phaseTwo() {
        ...
        if (optionalSample.isPresent()) {
            ...
            throw new IntegrationTestFailException("Sample does not exist, sample name: " + testSample2.getName(), Phase.TWO);
        }
        ...
        relations.add(Relationship.build(testSample3.getAccession(), "parent of", testSample2.getAccession()));
    ...
    protected void phaseThree() {
        ...
        if (optionalSample.isPresent()) {
            ...
            throw new IntegrationTestFailException("Sample does not exist, sample name: " + testSample1.getName(), Phase.THREE);
        }
        ...
        if (optionalSample.isPresent()) {
            ...
            throw new IntegrationTestFailException("Sample does not exist, sample name: " + testSample2.getName(), Phase.THREE);
        }
        ...
        if (samplePage.getMetadata().getTotalElements() != 1) {
            throw new IntegrationTestFailException("Unexpected number of results for attribute filter query: " + samplePage.getMetadata().getTotalElements(), Phase.THREE);
        }
        ...
        if (!targetAttribute.isPresent()) {
            throw new IntegrationTestFailException("Sample 2 should contain an attribute description", Phase.THREE);
        }
        ...
        if (!samplePage.getContent().stream().allMatch(r -> r.getContent().getAccession().equals(accession1) || r.getContent().getAccession().equals(accession2))) {
            throw new IntegrationTestFailException("Unexpected number of results for attribute filter query: " + samplePage.getMetadata().getTotalElements(), Phase.THREE);
        }
    ...
    protected void phaseFour() {
        ...
        if (optionalSample.isPresent()) {
            ...
            throw new IntegrationTestFailException("Sample does not exist, sample name: " + testSample1.getName(), Phase.FOUR);
        }
        ...
        if (optionalSample.isPresent()) {
            ...
            throw new IntegrationTestFailException("Sample does not exist, sample name: " + testSample2.getName(), Phase.FOUR);
        }
        ...
        if (optionalSample.isPresent()) {
            ...
            throw new IntegrationTestFailException("Sample does not exist, sample name: " + testSample3.getName(), Phase.FOUR);
        }
        ...
        PagedResources<Resource<Sample>> samplePage = client.fetchPagedSampleResource("",
                ...
                0, 10);
        if (samplePage.getMetadata().getTotalElements() < 1) {
            throw new IntegrationTestFailException("Unexpected number of results for date range filter query: " + samplePage.getMetadata().getTotalElements());
        }
        boolean match = samplePage.getContent().stream().anyMatch(resource -> resource.getContent().getAccession().equals(accession1));
        if (!match) {
            throw new IntegrationTestFailException("Returned sample doesn't match the expected sample " + testSample1.getAccession(), Phase.FOUR);
        }

        log.info("Getting sample 3 using filter on relation");
        Filter relFilter = FilterBuilder.create().onRelation("parent of").withValue(testSample2.getAccession()).build();
        samplePage = client.fetchPagedSampleResource("", Collections.singletonList(relFilter), 0, 10);

        if (samplePage.getMetadata().getTotalElements() < 1) {
            throw new IntegrationTestFailException("Unexpected number of results for relation filter query: " + samplePage.getMetadata().getTotalElements(), Phase.FOUR);
        }
        match = samplePage.getContent().stream().anyMatch(resource -> resource.getContent().getAccession().equals(accession3));
        if (!match) {
            throw new IntegrationTestFailException("Returned sample doesn't match the expected sample " + testSample3.getAccession(), Phase.FOUR);
        }


        log.info("Getting sample 2 using filter on inverse relation");
        Filter invRelFilter = FilterBuilder.create().onInverseRelation("parent of").withValue(testSample3.getAccession()).build();
        samplePage = client.fetchPagedSampleResource("", Collections.singletonList(invRelFilter), 0, 10);

        if (samplePage.getMetadata().getTotalElements() < 1) {
            throw new IntegrationTestFailException("Unexpected number of results for relation filter query. Expected more than zero but got " + samplePage.getMetadata().getTotalElements());
        }
        match = samplePage.getContent().stream().anyMatch(resource -> resource.getContent().getAccession().equals(accession2));
        if (!match) {
            throw new IntegrationTestFailException("Returned sample doesn't match the expected sample " + testSample3.getAccession(), Phase.FOUR);
        }
    }
    ...
    protected void phaseFive() {
        ...
        Filter domainFilter = FilterBuilder.create().onDomain(defaultIntegrationSubmissionDomain).build();
        PagedResources<Resource<Sample>> samplePage = client.fetchPagedSampleResource("",
                Collections.singletonList(domainFilter),
                0, 10);
        if (samplePage.getMetadata().getTotalElements() < 1) {
            throw new IntegrationTestFailException("Unexpected number of results for domain filter query: " + samplePage.getMetadata().getTotalElements(), Phase.FIVE);
        }
    }
