...
public class SampleGroupIntegration extends AbstractIntegration {
    ...
    if (optionalSample.isPresent()) {
      ...
      try {
        ...
        Sample testSampleWithAccession =
            Sample.Builder.fromSample(testSampleGroup)
        ...
        if (sampleResource.getContent().getAccession() == null) {
          throw new IntegrationTestFailException(
              "Failed to persist sample: " + testSample.getName());
        }
      } catch (Exception e) {
  ...
  protected void phaseTwo() {
    ...
    relationships.add(
        Relationship.build(
            optionalGroupSample.get().getAccession(),
            "Has member",
            optionalSample.get().getAccession()));
    testSampleGroup =
        Sample.Builder.fromSample(testSampleGroup)
            .withRelationships(relationships)
            .withAccession(optionalGroupSample.get().getAccession())
            .build();
