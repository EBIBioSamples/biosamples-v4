...
public class BigIntegration extends AbstractIntegration {
    ...
    protected void phaseTwo() {
        ...
        if (elapsedMs > timeout) {
            throw new RuntimeException("Took more than " + timeout + "ms to fetch highly-connected sample (" + elapsedMs + "ms)");
        }
        ...
        elapsedMs = (int) ((endTime - startTime) / 1000000l);
        if (elapsedMs > timeout) {
            throw new RuntimeException("Took more than  " + timeout + "ms to fetch all samples (" + elapsedMs + "ms)");
        }
