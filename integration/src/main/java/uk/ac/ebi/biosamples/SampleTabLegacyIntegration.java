...
public class SampleTabLegacyIntegration extends AbstractIntegration {
  ...
  public SampleTabLegacyIntegration(
    ...
    Pattern accessionPattern =
        Pattern.compile(sampleUtils.getAccessionPattern(), Pattern.MULTILINE);
  ...
  protected void phaseOne() {
    ...
    // RequestEntity.post(uriVa).contentType(MediaType.APPLICATION_JSON)
    //					.body(sampleTabString);
    //			ResponseEntity<String> response = restTemplate.exchange(request, String.class);
    //			log.info(""+response.getBody());
    //		});
    log.info("Testing SampleTab File validation");
    runCallableOnSampleTabFile(
        "/GSB-32.txt",
        sampletabFile -> {
    ...
    //			RequestEntity<String> request =
    // RequestEntity.post(uriAc).contentType(MediaType.APPLICATION_JSON)
    //					.body(sampleTabString);
    //			ResponseEntity<String> response = restTemplate.exchange(request, String.class);
    //			log.info(""+response.getBody());
    //
    ...
    //			RequestEntity<String> request =
    // RequestEntity.post(uriSb).contentType(MediaType.APPLICATION_JSON)
    //					.body(sampleTabString);
    //			ResponseEntity<String> response = restTemplate.exchange(request, String.class);
    //			log.info(""+response.getBody());
    //
    ...
    runCallableOnSampleTabFile(
        "/Implicit_relationship_sampletab.tab",
        sampletabFile -> {
          ResponseEntity<String> response = submitSampletabFile(sampletabFile);
          if (!response.getBody().contains("GSB-")) {
    ...
    runCallableOnSampleTabFile(
        "/accessioned_sampletab_with_implicit_relationships.tab",
        sampletabFile -> {
          ResponseEntity<String> response = accessionSampletabFile(sampletabFile);
                  ...
                  acc -> {
                    if (!response.getBody().contains(acc)) {
                      throw new RuntimeException(
                          "Accessioned sampletab does not contains expected accession " + acc);
                    }
                  }); //			Pattern accessionPattern =
          // Pattern.compile("(SAMEA\\d+)");
          //			Matcher m = accessionPattern.matcher(response.getBody());
          //			Set<String> accessionToFind= Stream.of("SAMEA7231988", "SAMEA7231987",
          // "SAMEA7231986", "SAMEA7231985").collect(Collectors.toSet());
          //			while(m.find()) {
          //				if (!accessionToFind.contains(m.group())) {
          ////					throw new RuntimeException("Found accession " + m.group() + " not part
          // of the
          // pre-accession sampletabs");
          //				} else {
          //					accessionToFind.remove(m.group());
          //				}
          //			}
          //			if (accessionToFind.size() > 0) {
          //				String accessionsNotFound = accessionToFind.stream().reduce("", (s, s2) ->
          // s +" ," +
          // s2);
          //				throw new RuntimeException("Response does not have expected accessions: "
          // +
          // accessionsNotFound);
          //			}
        });
    ...
    runCallableOnSampleTabFile(
        "/Invalid_implicit_relationship_sampletab.txt",
        sampletabFile -> {
          ResponseEntity<String> response = submitSampletabFile(sampletabFile);
    ...
    runCallableOnSampleTabFile(
        "/sampletab_with_escaped_newline.txt",
        sampletabFile -> {
          ResponseEntity<String> response = submitSampletabFile(sampletabFile);
  ...
  protected void phaseTwo() {
    ...
    Iterator<Resource<Sample>> sampleResourceIterator =
        client.fetchSampleResourceAll(null, Collections.singletonList(nameFilter)).iterator();
    ...
    if (sample.getRelationships().size() == 0) {
      throw new RuntimeException(
          "Sample foozitTest 1 should have some relationship but it doesn't");
    } else {
      ...
      for (Relationship r : sample.getRelationships()) {
        if (r.getTarget().equals(sampleAccession) && r.getType().equals("has member")) {
          ...
          if (!r.getSource().startsWith("SAMEG")) {
            throw new RuntimeException(
                "foozitTest 1 has 'has member' relationship to something not SAMEG");
          }
        }
    ...
    Optional<List<Attribute>> submissionIdAttribute =
        sampleUtils.getAttributesWithType(sample, "Submission identifier");
    if (!submissionIdAttribute.isPresent()) {
      throw new RuntimeException(
          "A 'Submission identifier' field should be present on a sample submitted using SampleTab");
    }

    if (submissionIdAttribute.get().size() != 1) {
      throw new RuntimeException(
          "The number of 'Submission identifier' attributes for the sample is not 1");
    }
    ...
    Filter submissionIdFilter =
        FilterBuilder.create().onAttribute("Submission identifier").withValue(submissionId).build();
    Iterable<Resource<Sample>> sampleResourceAll =
        client.fetchSampleResourceAll(null, Collections.singletonList(submissionIdFilter));
    List<String> submissionAccessions =
        StreamSupport.stream(sampleResourceAll.spliterator(), false)
  ...
  protected void phaseThree() {
    ...
    //				throw new RuntimeException("Couldn't find pre-accessioned sample SAMEA2186844 after
    // submission");
    //			}
    //		});
    ...
    runCallableOnSampleTabFile(
        ...
        sampletabFile -> {
          ...
          if (!clientSample.isPresent()) {
            throw new RuntimeException(
                "Couldn't find pre-accessioned sample SAMEA2186844 after submission");
          }
        });
  ...
  //	private void runCallableOnSampleTabResource(String resource, SampleTabStringCallback
  // callback)
  // {
  ...
  private ResponseEntity<String> postFileToUrl(ClassPathResource resource, URI uri) {
    ...
    RequestEntity<LinkedMultiValueMap> request =
        RequestEntity.post(uri).contentType(MediaType.MULTIPART_FORM_DATA).body(map);
  ...
  private void verify_relationships_in_accessioned_sampletab_are_exported_correctly() {
    ...
    Filter nameFilter = FilterBuilder.create().onName("Accessioned ValidOrigin").build();

    samplePage =
        client.fetchPagedSampleResource("*:*", Collections.singletonList(nameFilter), 0, 10);
    ...
    String submissionIdentifier =
        samplePage.getContent().stream()
            ...
            .getAttributes()
            .stream()
            .filter(attr -> attr.getType().equalsIgnoreCase("submission identifier"))
            .findFirst()
            .get()
            .getValue();
    Filter attrFilter =
    ...
    samplePage =
        client.fetchPagedSampleResource("*:*", Collections.singletonList(attrFilter), 0, 10);

    if (samplePage.getMetadata().getTotalElements() != 5) {
      throw new RuntimeException(
          "Unexpected number of samples found with Submission identifier GSB-9191");
    }
  ...
  private void
    ...
    samplePage = client.fetchPagedSampleResource("*:*", Collections.singleton(nameFilter), 0, 1);
    if (samplePage.getMetadata().getTotalElements() != 1) {
    ...
    Filter derivedSampleName = FilterBuilder.create().onName("DerivedFromValidOrigin").build();

    samplePage =
        client.fetchPagedSampleResource(
            "*:*",
            Stream.of(derivedFromFilter, derivedSampleName).collect(Collectors.toList()),
            0,
            1);

    if (samplePage.getMetadata().getTotalElements() != 1) {
      throw new RuntimeException(
          "Unexpected number of samples found with name DerivedFromValidOrigin and "
              + "relationship");
    }
  ...
  private void verify_external_references_was_picked_up_from_sampletab() {
    ...
    List<String> externalReferencesURL =
        sample.get().getContent().getExternalReferences().stream()
  ...
  private void verify_samples_with_escaped_newline_characters_are_kept_the_same() {
    ...
    Filter nameFilter = FilterBuilder.create().onName(sampleNameToCheck).build();
    PagedResources samplePage =
        client.fetchPagedSampleResource("*:*", Collections.singletonList(nameFilter), 0, 1);
    ...
    for (Attribute attr : sample.getCharacteristics()) {
      if (attr.getType().equalsIgnoreCase("description")) {
        descriptionCorrect =
            attr.getValue()
                .equals("This sample has and escaped newline character \\n inside the description");
    ...
    if (!descriptionCorrect) {
      throw new RuntimeException(
          "Description with newline escaped character is not found correctly");
    }

    if (!diseaseCorrect) {
      throw new RuntimeException(
          "Disease attribute with newline escaped character is not found correctly");
    }
  }
}
