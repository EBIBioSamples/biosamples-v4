...
import uk.ac.ebi.biosamples.model.filters.FilterBuilder;
...
public class RestSearchIntegration extends AbstractIntegration {;
	...
	protected void phaseFour() {
		Sample test6 = getSampleTest6();
		Filter attributeFilter = FilterBuilder.getFilterBuilderForAttributeField("MySpecialAttributeType").withValue("MySpecialAttributeValue").build();
		...
		Filter dateFilter = FilterBuilder.releaseDateFilterBuilder().from(fromDateTime).to(fromDateTime.plusSeconds(2)).build();
