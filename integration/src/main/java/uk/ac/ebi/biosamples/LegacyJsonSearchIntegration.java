...
public class LegacyJsonSearchIntegration extends AbstractIntegration {
  ...
  public LegacyJsonSearchIntegration(
    ...
    this.jsonSearchTester =
        new LegacyJsonSearchTester(log, client, this.restTemplate, this.integrationProperties);
  }
  ...
  /**
   * Check retrieve of samples/groups from corresponding endpoint, and containing correct
   * informations
   */
  @Override
  ...
  private class LegacyJsonSearchTester {
    ...
    public void registerTestSamplesInBioSamples() {
      ...
      Optional<Resource<Sample>> optional =
          this.client.fetchSampleResource(groupContainegSAMEA911.getAccession());
      ...
      // created
      // automatically
      if (!groupContainegSAMEA911
          .getAccession()
          .equals(groupResource.getContent().getAccession())) {
      ...
      Resource<Sample> sampleWithinGroupResource =
          this.client.persistSampleResource(sampleWithinGroup);
      // The result and the submitted will not be equal because of the new inverse relation
      // created
      // automatically
      ...
      Resource<Sample> sampleWithFullDetailsResource =
          this.client.persistSampleResource(sampleWithFullDetails, false, true);
      // The result and the submitted will not be equal because of the new inverse relation
      // created
      // automatically
      if (!sampleWithFullDetails
          .getAccession()
          .equals(sampleWithFullDetailsResource.getContent().getAccession())) {
        throw new RuntimeException("Expected response to equal submission");
      }
      ...
      Resource<Sample> groupWithFullDetailsResource =
          this.client.persistSampleResource(groupWithFullDetails, false, true);
      // The result and the submitted will not be equal because of the new inverse relation
      // created
      // automatically
      if (!groupWithFullDetails
          .getAccession()
          .equals(groupWithFullDetailsResource.getContent().getAccession())) {
        throw new RuntimeException("Expected response to equal submission");
      }
    }
    ...
    public void itShouldFindTheSampleByAccessionUsingLegacyEndpoint() {
      ...
      log.info(
          String.format("Searching sample %s using legacy json api", testSample.getAccession()));
      UriComponentsBuilder uriBuilder =
          UriComponentsBuilder.fromUri(integrationProperties.getBiosamplesLegacyJSONUri());
    ...
    public void itShoudlFindAllSampleDetailsInTheJSON() {
      ...
      log.info(
          "Verifying sample "
              ...
              + " contains all details in the legacy JSON api");
      UriComponentsBuilder uriBuilder =
          UriComponentsBuilder.fromUri(integrationProperties.getBiosamplesLegacyJSONUri());
      uriBuilder.pathSegment("samples", testSample.getAccession());
    ...
    public void itShouldBeAbleToMoveUsingLinks() {
      ...
      Traverson.TraversalBuilder traversalBuilder =
          new Traverson(uriBuilder.build().toUri(), HAL_JSON).follow("relations", "groups");
      ...
      String sampleJson =
          traversalBuilder
              .follow("$._embedded.groupsrelations[0]._links.samples.href")
              .toObject(String.class);
      assertEquals(
          JsonPath.parse(sampleJson).read("$._embedded.samplesrelations[0].accession"),
          testSample.getAccession());

      String externalLinkJson =
          traversalBuilder
              .follow("$._embedded.samplesrelations[0]._links.externalLinks.href")
              .toObject(String.class);
      assertEquals(
    ...
    public void itShouldFindSampleSearchingByAccession() {
      ...
      Traverson.TraversalBuilder traversalBuilder =
          new Traverson(uriBuilder.build().toUri(), HAL_JSON)
              .follow("samples", "search")
              .follow(
                  Hop.rel("findByAccession").withParameter("accession", testSample.getAccession()));

      assertEquals(
          traversalBuilder.toObject("$._embedded.samples[0].accession"), testSample.getAccession());
    ...
    public void itShouldFindSampleSearchingByFirstSampleInGroup() {
      ...
      Traverson.TraversalBuilder traversalBuilder =
          new Traverson(integrationProperties.getBiosamplesLegacyJSONUri(), HAL_JSON)
              .follow("samples", "search")
              .follow(
                  Hop.rel("findFirstByGroupsContains")
    ...
    public void itShouldFindSampleSearchingByText() {
      ...
      assertThat(
          traversalBuilder.toObject(
              "$._embedded.samples[?(@.accession=='" + testSample.getAccession() + "')].accession"),
          contains(testSample.getAccession()));
    }
    ...
    public void itShouldFindSampleSearchingByAccessionAndGroup() {
      ...
      Traverson.TraversalBuilder traversalBuilder =
          new Traverson(integrationProperties.getBiosamplesLegacyJSONUri(), HAL_JSON)
              .follow("samples", "search")
              .follow(
                  Hop.rel("findByAccessionAndGroups")
    ...
    public void itShouldFindGroupSearchingByAccession() {
      ...
      Traverson.TraversalBuilder traversalBuilder =
          new Traverson(integrationProperties.getBiosamplesLegacyJSONUri(), HAL_JSON)
              ...
              .follow(
                  Hop.rel("findByAccession")
                      .withParameter("accession", testGroup.getAccession())
                      .withParameter("size", 100));

      assertEquals(
          traversalBuilder.toObject("$._embedded.groups[0].accession"), testGroup.getAccession());
  ...
  private static class TestSampleGenerator {
    ...
    private static final String submissionDomain = "self.BiosampleIntegrationTest";
    ...
    public static Sample getSampleMemberOfGroupWithExternalRelations() {
      ...
      attributes.add(
          Attribute.build(
              "organism",
              "Very special Homo Sapiens",
              "http://purl.obolibrary.org/obo/NCBITaxon_9606",
              null));
      SortedSet<ExternalReference> externalReferences = new TreeSet<>();
      externalReferences.add(ExternalReference.build("http://hPSCreg.eu/cell-lines/PZIF-001"));
      SortedSet<Relationship> relationships = new TreeSet<>();
      //            relationships.add(Relationship.build("SAMEG199", "has member",
      // "SAMEA911"));

      //            return Sample.build(name, accession, "self.BiosampleIntegrationTest",
      // release,
      // update, attributes, relationships, externalReferences);
    ...
    public static Sample getSampleFullOfDetails() {
      ...
      //            return Sample.build(name, accession, domain, release, update, attributes,
      // relationships, externalReferences, organizations, contacts, publications);
      return new Sample.Builder(name, accession)
          .withDomain(domain)
          .withRelease(release)
          .withUpdate(update)
