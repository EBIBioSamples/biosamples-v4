  ...
  private static class XmlSearchTester {
    ...
    public void triesToFindSampleUsingLegacyEndpoint() {
      ...
      UriComponentsBuilder uriBuilder =
          UriComponentsBuilder.fromUri(integrationProperties.getBiosamplesLegacyXMLUri());
      uriBuilder.pathSegment("samples", test1.getAccession());

      RequestEntity<?> request =
          RequestEntity.get(uriBuilder.build().toUri())
    ...
    public void triesAndFailToFindPrivateSampleUsingLegacyEndpoint() {
      ...
      log.info(
          String.format(
              "Searching private sample %s using legacy xml api", privateSample.getAccession()));
      ...
      try {
        RequestEntity<?> sampleRequestEntity =
            RequestEntity.get(uriBuilder.build().toUri()).accept(MediaType.TEXT_XML).build();

        ResponseEntity<String> sampleResponseEntity =
            restTemplate.exchange(sampleRequestEntity, String.class);
        if (sampleResponseEntity.getStatusCode().equals(HttpStatus.OK)) {
          throw new RuntimeException(
              String.format(
                  "Sample %s should be not available through the legaxy xml api",
    ...
    public void triesToFindGroupUsingLegacyEndpoint() {
      ...
      ResponseEntity<String> response =
          restTemplate.exchange(
              testGroupsEndpoint.toUriString(),
              HttpMethod.GET,
      ...
      if (!response.getStatusCode().is2xxSuccessful()) {
        throw new RuntimeException(
            "Something went wrong while retrieving group from legacy XML endpoint");
      }
    ...
    public void triesToFindSamplePartOfGroupUsingLegacyEndpoint() {
      ...
      ResponseEntity<String> response =
          restTemplate.exchange(
              testSamplesEndpoint.toUriString(),
              HttpMethod.GET,
              new HttpEntity<>(xmlHeaders),
              String.class);
      ...
      if (!response.getStatusCode().is2xxSuccessful()) {
        throw new RuntimeException(
            "Something went wrong while retrieving sample from legacy XML endpoint");
      }

      if (!XmlMatcher.matchesSampleInGroup(response.getBody(), sampleWithinGroup)) {
        throw new RuntimeException(
            "The returned sample doesn't match the expected characteristics");
      }
    }

    public void failsToAccessLegacyEndpointUsingJsonHeader() {
      log.info(
          "Try to generate a NOT ACCEPTABLE error using legacy xml samples end-point with application/json accept header");
      UriComponentsBuilder uriBuilder =
          UriComponentsBuilder.fromUri(integrationProperties.getBiosamplesLegacyXMLUri());
      ...
      try {
        ...
        // ex.getResponseHeaders().getContentType().includes(MediaType.APPLICATION_JSON);
        //                if (!expectedResponse) {
        //                    throw new RuntimeException("Excepted response content-type
        // doesn't
        // match application/json", ex);
        //                }
      }
    }
    ...
    public void getsResultSummaryAsResultOfSearchingSampleInGroup() {
      ...
      ResponseEntity<ResultQuery> response =
          restTemplate.exchange(
              testGroupsEndpoint.toUriString(),
              HttpMethod.GET,
              new HttpEntity<>(xmlHeaders),
      ...
      Optional<BioSample> xmlSample =
          resultQuery.getBioSample().stream()
              .filter(bioSample -> bioSample.getId().equals(sampleWithinGroup.getAccession()))
              .findFirst();
      xmlSample.orElseThrow(
          () ->
              new RuntimeException(
                  "The legacy XML result query doesn't contain the expected group"));
    }
    ...
    public void searchesForSamplesUsingDateRangesOnLegacyEndpoint() {
      ...
      if (!response.getStatusCode().is2xxSuccessful()) {
        throw new RuntimeException(
            "Something went wrong while retrieving results from legacy XML endpoint");
      }
    ...
    public void findSamplesUsingNCBIQueryStyle() {
      ...
      //                    .filter(bioSample ->
      // bioSample.getId().equals(testSample.getAccession()))
      //                    .findFirst();
      //            if (!xmlSample.isPresent()) {
      //                throw new RuntimeException("The legacy XML result query doesn't
      // contain the
      // expected sample");
      //            }
    }
    ...
    public void doesNotFindContactInformationInSample() {
      ...
      log.info(
          String.format("Searching sample %s using legacy xml api", testSample.getAccession()));
      UriComponentsBuilder uriBuilder =
          UriComponentsBuilder.fromUri(integrationProperties.getBiosamplesLegacyXMLUri());
      ...
      if (!responseEntity
          .getBody()
          .contains(String.format("id=\"%s\"", testSample.getAccession()))) {
        throw new RuntimeException("Response body doesn't match expected sample");
      }
      ...
      log.info(
          String.format(
              "Sample %s does not contains Person element in legacy XML api as expected",
    ...
    public void findsAllFieldsInLegacyGroup() {
      ...
      //            log.info(String.format("Sample %s does not contains Person element in
      // legacy XML
      // api as expected",
      //                    testSample.getAccession()));
    }
    ...
    public void findOnlySamplesStartingWithSAMEA() {
      ...
      ResponseEntity<ResultQuery> responseEntity =
          restTemplate.exchange(request, ResultQuery.class);
      assert (responseEntity.getStatusCode().is2xxSuccessful());
      ResultQuery results = responseEntity.getBody();
      Optional<BioSample> notExpectedBiosamples =
          results.getBioSample().stream()
              .filter(bioSample -> !bioSample.getId().startsWith("SAMEA"))
              .findAny();
      if (notExpectedBiosamples.isPresent()) {
        throw new RuntimeException(
            "An unexpected sample with accession "
                + notExpectedBiosamples.get().getId()
                + " "
    ...
    public void findSamplesReleasedWithinSpecificDate() {
      ...
      Sample testSampleShouldNotBeReturned =
          TestSampleGenerator.getSampleReleasedExaclyTheDayAfterSAMD0912312();
      ...
      Optional<BioSample> expectedSample =
      ...
      if (expectedSample.isEmpty()) {
        throw new RuntimeException(
            "Sample "
                + testSample.getAccession()
                + " not found when searching within a single specific date");
      }

      Optional<BioSample> unexpectedSample =
          result.getBioSample().stream()
      ...
      if (unexpectedSample.isPresent()) {
        throw new RuntimeException(
            "Sample "
                + testSampleShouldNotBeReturned.getAccession()
                + " should not be found because not part of the daterange");
      }
    }
  }
